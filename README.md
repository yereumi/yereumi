<!--
**yereumi/yereumi** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

# 기술과 책임을 함께 고민하는 개발자, 신예지

[![Contact](https://skillicons.dev/icons?i=gmail)](lovelyyeji01@gmail.com) &nbsp;
[![Linkedin](https://skillicons.dev/icons?i=linkedin)](https://www.linkedin.com/in/yereumi)

## 📍 Introduction

<!--
[![Mail](https://img.shields.io/badge/Mail-EA4335?style=flat&logo=Gmail&logoColor=white)]()
[![Blog](https://img.shields.io/badge/Blog-20C997?style=flat&logo=Velog&logoColor=white)](https://velog.io/@yereumi)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=flat&logo=Instagram&logoColor=white)](https://www.instagram.com/ye.reumi/)
-->

### 1. 빠르게 성장합니다
프로젝트 배포 과정에서 장애가 발생했을 때, 인프라 담당자가 부재한 상황에서도 AWS 공식 문서와 레퍼런스를 빠르게 학습하여 직접 **5시간만에** 해결한 경험이 있습니다.
또한 4주 프로젝트에서 처음 사용해본 Graph DB(Neo4j)를 단기간에 학습하고 Cypher 쿼리 기반 최적화를 적용하여 응답 속도를 **약 60% 이상** 개선했습니다.

### 2. 사용자 친화적인 서비스를 지향합니다
**약 100명의 베타 테스터**를 대상으로 성능 테스트를 진행하며, 피드백과 로그 분석을 통해 캐시 구조와 쿼리 로직을 최적화했습니다.
그 결과, 응답 속도를 **약 90% 이상** 개선하며 안정적이고 빠른 사용자 경험을 제공했습니다.

### 3. 성실합니다
학창시절 12년 개근과 대학교 우등 졸업, 그리고 LG유플러스 교육 최우수상 수상으로 성실함을 증명해왔습니다.

#### 앞으로도 성실함과 꾸준한 학습을 바탕으로 기술과 책임을 함께 고민하는 개발자로 성장하겠습니다.

## 🛠️ Tech Stack

[![My Skills](https://skillicons.dev/icons?i=java,spring,mysql,redis,docker,nginx,ubuntu,githubactions,aws,prometheus,grafana)](https://skillicons.dev)

## ⚙️ Algorithm
[![Solved.ac Profile](http://mazassumnida.wtf/api/v2/generate_badge?boj=lovelyyeji01)](https://solved.ac/lovelyyeji01) 
